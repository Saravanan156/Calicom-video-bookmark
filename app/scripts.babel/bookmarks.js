(function () {
    ...
    BookMarksController.$inject = ['$mdDialog', '$timeout'];
    function BookMarksController($mdDialog, $timeout) {
        var vm = this;
        vm.bookmarks = [];
        ...
        vm.showEditPrompt = function (bookmark, ev) {
            // Appending dialog to document.body to cover sidenav in docs app
            var confirm = $mdDialog.prompt()
                .title('Edit Your Bookmark Here')
                .textContent(bookmark.title)
                .ariaLabel('Bookmark Name')
                .placeholder('Bookmark name here...')
                .initialValue(bookmark.text)
                .targetEvent(ev)
                .theme('altTheme')
                .ok('Save')
                .cancel('Cancel');
            ...
            $mdDialog.show(confirm).then(function (result) {
                _.map(vm.bookmarks, function (items) {
                    if (items.id == bookmark.id) {
                        items.text = result;
                    }
                });
